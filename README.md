...
- **No double-counting with the compensation line:** suppliers bill the electricity subsidy *through* the compensation mechanism (“Mécanisme de compensation A −0,0371/kWh” = −€0.04/kWh incl. VAT) instead of the base compensation credit. The preset therefore suspends the base Compensation Fund credit on subsidised kWh — it is not added on top. Subsidy quantities always reconcile to the billed grid import: 15-minute intervals only split the period, they never redefine the total.
- **Two supported setups, same total:** (a) keep the base Compensation Fund rate and the preset enabled — the estimate lands within 2 cents of the printed bill (the supplier rounds its per-unit rate to four decimals); (b) for a cent-exact SUDenergie match, enter the supplier's published rate (−0,0371/kWh) in Compensation Fund and disable the preset. Never combine (b) with an enabled preset — the preset replaces that line.
