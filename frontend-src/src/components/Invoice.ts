...
export function renderInvoice(state: AppState): string {
  ...
  const allocatedExportKwhPerMeter = hasMultipleRates && resolvedRates.length > 0
    ...
    : soldToMarket;
  const feedInRows = resolvedRates.map((r) => ({
    ...r,
    exportKwh: allocatedExportKwhPerMeter,
  ...
  const selfConsumptionExportEquivalent = selfConsumed * avgFeedInRate;
        ...
        <table class="invoice-table">
          ...
          <tbody>
            ...
            <tr class="revenue-row">
              ...
              <td style="text-align: right;">${fmtNum(production)} kWh</td>
            </tr>
            <tr class="revenue-row">
              <td>Autoconsumed / used at home</td>
              <td style="text-align: right;">${fmtNum(selfConsumed)} kWh avoided grid purchases</td>
              <td style="text-align: right;">${fmt(totalSelfConsumedSavings)} saved</td>
            </tr>
            <tr class="revenue-row">
              <td>Export sold</td>
              <td style="text-align: right;">${fmtNum(soldToMarket)} kWh sent to grid</td>
              <td style="text-align: right;">${fmt(feedInRevenue)} earned</td>
            </tr>
            ${hasAvoidedExceedanceSavings ? `
            <tr class="revenue-row">
              <td>Reference exceedance avoided</td>
              <td style="text-align: right;">${fmtNum(avoidedExceedanceKwh)} kWh above the reference stayed covered by solar</td>
            ...
            ${feedInRows.map((r) => `
            <tr class="revenue-row">
              <td>Exported (${hasMultipleRates ? r.shortId : fmtNum(r.exportKwh) + " kWh"})</td>
              <td style="text-align: right;">${fmtNum(r.exportKwh)} kWh<br/>${r.label}<br/>${fmtNum(r.rate, 4)} ${currency}/kWh</td>
      ...
      <div class="card solar-revenue-card">
        ...
        <div class="solar-revenue-summary">
          ...
          <div class="solar-stat">
            <div class="solar-stat-value">${fmtNum(production)} kWh</div>
            <div class="solar-stat-label">Solar produced</div>
          </div>
          <div class="solar-stat">
            <div class="solar-stat-value">${fmt(totalSelfConsumedSavings)}</div>
            <div class="solar-stat-label">Saved by autoconsuming ${fmtNum(selfConsumed)} kWh</div>
          </div>
          <div class="solar-stat">
          ...
          <div class="solar-stat">
            ...
            <div class="solar-stat-label">Earned by selling ${fmtNum(soldToMarket)} kWh</div>
          </div>
        </div>
        ...
        <table class="invoice-table solar-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style="text-align: right;">Compared with selling the same ${fmtNum(selfConsumed)} kWh at ${fmtNum(avgFeedInRate, 4)} ${currency}/kWh</td>
            ...
            <tr>
              ...
              <td style="text-align: right;">${fmtNum(avoidedExceedanceKwh)} kWh above the reference power level</td>
            ...
            ${feedInRows.map((r) => `
            <tr>
              <td>Sold to grid ${hasMultipleRates ? `(${r.shortId})` : `(${fmtNum(r.exportKwh)} kWh)`}</td>
              <td style="text-align: right;">${fmtNum(r.exportKwh)} kWh<br/>${r.label}<br/>${fmtNum(r.rate, 4)} ${currency}/kWh</td>
        ...
        <p class="muted" style="margin-top: var(--sp-3); font-size: var(--text-xs); line-height: var(--lh-relaxed);">
          ...
          ${hasMultipleRates ? "Displayed per-meter feed-in kWh are currently equal-split estimates because per-meter export data is not yet available." : ""}
        </p>
      </div>
      ` : ""}
    </section>
