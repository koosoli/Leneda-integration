...
export function renderInvoice(state: AppState): string {
  ...
  const directSolarToHome = Math.min(totalSolarToHome, Math.max(
    0,
    d.direct_solar_to_home ??
      (d.self_consumed && d.self_consumed > 0 ? d.self_consumed : production - soldToMarket),
  ));
  ...
  const selfConsumed = allocatedSelfConsumed;
  const avoidedImportRate = config.energy_variable_rate + config.network_variable_rate + config.electricity_tax_rate + config.compensation_fund_rate;
  ...
  const perSystemSolarRows = feedInRows.map((row) => {
    const selfUseSavings = row.selfConsumedKwh * avoidedImportRateWithVat;
    const selfUseVsExport = selfUseSavings - row.exportEquivalentForSelfUse;
    return {
      ...row,
      selfUseSavings,
      selfUseVsExport,
      totalTrackedValue: selfUseSavings + row.revenue,
    };
  });
  const hasPerSystemSolarBreakdown = usesPrioritySolarAllocation && perSystemSolarRows.length > 0;
    ...
    ? `
            ...
            <tr>
              ...
              <td style="text-align: right;">
                ...
                <span class="${fmtDeltaClass(r.selfUseVsExport)}">${fmtSigned(r.selfUseVsExport)}</span> self-use vs export<br/>
                <span class="muted">${fmt(r.selfUseSavings)} kept value + ${fmt(r.revenue)} sold</span>
              </td>
            </tr>
            `).join("")}
            <tr class="subtotal-row">
              <td colspan="2"><strong>Tracked per-system value</strong></td>
              <td style="text-align: right;"><strong>${fmt(perSystemSolarRows.reduce((sum, r) => sum + r.totalTrackedValue, 0))}</strong></td>
            </tr>
      `
    : "";
    ...
    <section class="invoice-view">
      ...
      <div class="section-header invoice-section-header">
        <div class="invoice-header-top">
          <div>
            <h2>Supplier Bill Estimate &mdash; ${rangeLabel}</h2>
        ...
        <table class="invoice-table">
          ...
          <tbody>
            ...
            <tr class="revenue-row">
              ...
              <td style="text-align: right;">${fmtKwh(selfConsumed)} kWh from your own production avoided grid purchases</td>
              <td style="text-align: right;">${fmt(totalSelfConsumedSavings)} saved</td>
            </tr>
            ${communitySolarToHome > 0 ? `
            <tr class="revenue-row">
              <td>Additional solar received</td>
              <td style="text-align: right;">${fmtKwh(communitySolarToHome)} kWh covered at home from shared/community solar</td>
      ...
      <div class="card invoice-footer">
        <p class="muted" style="line-height: var(--lh-relaxed);">
          <strong>Combined Net Energy Position: ${fmt(netTotal + gasTotalCosts)}</strong>
          (Electricity net position: ${fmt(netTotal)} + Gas supplier estimate: ${fmt(gasTotalCosts)})
        </p>
      </div>
      ` : ""}
      ...
      <div class="card solar-revenue-card">
        <h3 class="card-title"><span class="title-icon">☀️</span> Solar Panel Value &mdash; ${rangeLabel}</h3>
        <div class="solar-revenue-summary">
          ...
          <div class="solar-stat">
            ...
            <div class="solar-stat-label">Saved by using ${fmtKwh(selfConsumed)} kWh of your own solar at home</div>
          </div>
          <div class="solar-stat">
